...
│     ├── /api/pipeline/*  → PipelineModule (pipeline snapshot + owner-only bot commands, tracker.db)
...
- `tracker.db` — bot's DB, mounted via Docker volume. Bot owns schema.
  PipelineModule's snapshot opens its own `readonly` handle on it and never
  writes or migrates through it (every other module goes through
  TrackerService's read-write one — including PipelineModule's own
  `bot_commands` inserts, the /pipeline page's owner-only bot commands);
...
GET /api/pipeline/snapshot?days=1   → hunt / apply / result tiers + events footer
                                      ...
                                      Also serves hunt.live {active,last} (bot `hunt_live`),
                                      hunt.next {hunt,retry,updated_at} (config KV `bot_state.*`)
                                      and top-level control {sources, commands (10 newest
                                      bot_commands)} — missing table/key → null, never 0.
POST /api/pipeline/commands         { kind: 'hunt'|'retry_failed'|'check_expired', sources?: string[]|null }
                                      ...
                                      read-write handle, one IMMEDIATE transaction); the bot's
                                      3 s drain claims and runs it. sources: hunt only (400 on
                                      other kinds), each name must be in config KV
                                      `bot_state.sources` (400 unknown; key missing → 503 "bot
                                      state unavailable"); absent/null = every source. 409 for
                                      hunt/retry_failed while a hunt_live row is unfinished OR a
                                      hunt/retry_failed command is pending/running (the bot's one
                                      `_hunt_lock` — disabled, not queued); check_expired → 409
                                      only while another check_expired is pending/running.
GET  /api/pipeline/commands/:id     → { id, kind, payload (parsed), status, error, result (raw
                                      text), created_at, started_at, finished_at }; owner-only,
                                      404 if absent. Status: pending → running → done | error,
                                      or rejected (error = reason).
...
| 2026-09-24 | opus | docs/PIPELINE_VIZ_PLAN.md M2, API side: `GET /api/pipeline/snapshot?days=1..30` (new `PipelineModule`, no owner gate — plan open question 4). `src/pipeline/pipeline-snapshot.ts` ports the bot's reference `tools/pipeline_snapshot.py` against its `docs/PIPELINE_SNAPSHOT_CONTRACT.md` @ bot master 5c35447 (incl. #296 `run.refine_target`/`refine_max_rounds` and #297 `events[].details`, parsed from the FULL payload column), minus the contract's "Not in the contract" keys (`next_slot`, `coverage`, local-config keys, `at` display strings, the 80-char `events[].payload`); `sent-parse.ts` ports `hunter/sent_parse.py`; `snapshot-time.ts` computes the Warsaw calendar-day window explicitly via Intl (never the process TZ, DST-tested) and a Python-compatible `round()` (ties to even). Own `readonly` better-sqlite3 handle opened lazily (TrackerService's is read-write and migrates), all reads in ONE read transaction; every optional table/column is probed (`sqlite_master`/`PRAGMA table_info`) and degrades to `null`, missing `applications` → 503. Two gaps this port found were adopted into the contract in #297: user-scoped events footer + open-run lookup (the tool's footer was global and its company subquery could pick another user's row) and a raw `ts` on `run.last_event`/`run.refine_progress`. Remaining deliberate hardening over the tool: `fail_count`/`claimed_at`/`cost_usd` probed (the tool selects them unconditionally) and a partially-migrated optional table counts as missing (the tool probes only hunt_runs, and not its `id`). `apply.failures.log_records` reads `APPLY_FAILURES_LOG_PATH` — unset in prod because the bot's `logs/` isn't mounted here, so it is `null`. Fixtures in `test/fixtures/pipeline_snapshot/` (`schema.sql` generated from the bot's own DDL, `fixture.sql`/`expected.json` copied from the contract); the unit spec asserts exact deep equality with the contract's expected.json (after dropping the not-served keys) under an injected clock, the e2e re-owns the fixture rows by two real accounts and checks the same through JWT + scoping + `days` validation. Snapshot over the fixture: ~6 ms cold. `source_runs.ts` has no index (per-source ring buffer, small scan). |
| 2026-09-25 | opus | /pipeline control, API side (plan "live loaders, next-run time, action buttons", PR 2; branch `feat/pipeline-control` on top of `feat/pipeline-snapshot`). `bot_commands` DDL (the shared contract, byte-identical to the bot's) added idempotently to `tracker-migrations.ts` next to `profile_jobs`. Snapshot: `hunt.live` = newest unfinished (preferring one past `waiting`, so a hunt queued behind the lock never hides the one fetching) / newest finished `hunt_live` row (`sources` parsed; a garbled value → `[]`), `hunt.next` from the `bot_state.next_hunt` / `next_retry` / `updated_at` config keys, top-level `control` = `bot_state.sources` + the 10 newest `bot_commands` (payload parsed, garbled → null; no user_id/result). Every one of them is `null` for a missing or partially-migrated table and per-field `null` for a missing/garbled key — never 0. `POST /api/pipeline/commands` + `GET /api/pipeline/commands/:id` (owner-only, `PipelineCommandsService`, writes via TrackerService's RW handle in one IMMEDIATE transaction so the busy check and the insert can't interleave with a concurrent POST; transient SqliteError → 503). Contract fixtures: the new tables/rows/keys are appended to `schema.sql`/`fixture.sql`/`expected.json` from the plan's shared contract, marked as pending the bot contract update (README). Known edge left to the bot: a `hunt_live` row the bot never finishes (crash mid-hunt) keeps hunt/retry POSTs at 409 until the bot stamps it; the plan's startup cleanup covers `bot_commands` only. |
