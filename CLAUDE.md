...
- `tracker.db` — bot's DB, mounted via Docker volume. Bot owns schema;
  ...
  Sheet) plus the API-owned `app_status`/`note` columns (never mirrored) and,
  as of 2026-09-14, the bot-owned `outcome_label`/`outcome_at` pair (set-only,
...
PATCH  /api/applications/:id    { sent?, toLearn?, reapplication?, appStatus?, note? }
                                 (appStatus derives sent/outcome_label/outcome_at when
                                  sent is absent from the body — see 2026-09-14 work log
                                  entry and src/tracker/app-status.ts for the mapping)
